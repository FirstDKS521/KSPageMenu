...
#import "KSPageSegmentedTitleView.h"

//调用setViewControllers方法时，可能会用到延时
static float SetViewControllersMethodDelay = 0.1;

typedef void(^KSContentScollBlock)(BOOL scrollEnabled);
@interface KSPageContentView : UIView

@property (nonatomic, strong) KSContentScollBlock scrollBlock;

@end

@implementation KSPageContentView

//兼容和子view滚动冲突问题
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event {
    UIView *view =  [super hitTest:point withEvent:event];
    BOOL pageViewScrollEnabled = !view.ks_letMeScrollFirst;
    if (self.scrollBlock) {
...
@interface KSPageViewController ()<UIPageViewControllerDelegate, UIPageViewControllerDataSource, UIScrollViewDelegate,  KSPageTitleViewDataSrouce, KSPageTitleViewDelegate>

//所有的子视图，都加载在contentView上
@property (nonatomic, strong) KSPageContentView *contentView;
//标题
...
- (void)initUIWithConfig:(KSPageMenuConfig *)config {
    //保存配置
    self.config = config;
    
    //创建contentview
    self.contentView = [[KSPageContentView alloc] init];
    ...
    if (config.titleViewStyle == KSPageTitleViewStyleSegmented) {
        self.titleView = [[KSPageSegmentedTitleView alloc] initWithConfig:config];
    }
    ...
    //兼容全屏返回手势识别
    self.scrollView.ks_otherGestureRecognizerBlock = ^BOOL(UIGestureRecognizer * _Nonnull otherGestureRecognizer) {
...
- (void)pageViewController:(UIPageViewController *)pageViewController didFinishAnimating:(BOOL)finished previousViewControllers:(NSArray<UIViewController *> *)previousViewControllers transitionCompleted:(BOOL)completed {
    ...
    _selectedIndex = [self.allTitleArr indexOfObject:vc.ks_title];
    //标题居中
...
//titleview数据源方法
- (KSPageTitleCell *)pageTitleViewCellForItemAtIndex:(NSInteger)index {
...
- (BOOL)switchToViewControllerAdIndex:(NSInteger)index animated:(BOOL)animated {
    ...
    //如果正在加载中 返回
    if (self.pageVCAnimating && self.config.titleViewStyle == KSPageTitleViewStyleBasic) {return NO;}
...
- (KSPageTitleCell *)dequeueReusableTitleViewCellWithIdentifier:(NSString *)identifier forIndex:(NSInteger)index {
...
- (UIViewController *)viewControllerForIndex:(NSInteger)index {
    ...
    //当前标题
    NSString *currentTitle = currentVC.ks_title;
    //目标切换位置标题
    ...
    for (UIViewController *vc in self.shownVCArr) {
        if ([vc.ks_title isEqualToString:targetTitle]) {
            return vc;
        }
    }
    ...
    //设置扩展id，用户定位对应视图控制器
    vc.ks_title = [self titleForIndex:index];
    //设置试图控制器标题
